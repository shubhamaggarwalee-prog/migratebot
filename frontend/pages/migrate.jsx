...
function StepSource({ onNext }) {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/upload-source`, {
        ...
        headers: {
          ...
          Authorization : `Bearer ${(() => { try { return localStorage.getItem('mb_token'); } catch { return null; } })()}`,
        },
  ...
  const handleNext = () => {
    ...
    if (source === 'replit' && replitToken) { try { sessionStorage.setItem('mb_replit_token', replitToken); } catch { /* sandboxed */ } }
    onNext();
  };
...
function StepConfigure({ onNext, onBack, setMigId }) {
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      const authHeader = `Bearer ${(() => { try { return localStorage.getItem('mb_token'); } catch { return null; } })()}`;
